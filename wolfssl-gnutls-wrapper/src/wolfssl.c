...
static int wolfssl_pk_get_bits(void **_ctx, unsigned int* bits)
{
    ...
    switch (ctx->algo) {
        ...
        case GNUTLS_PK_RSA_OAEP:
            *bits = wc_RsaEncryptSize(&ctx->key.rsa);
            break;
        case GNUTLS_PK_DH:
            {
                word32 pSz, qSz, gSz;
    ...
    const void *x)
{
    ...
    if (!data || !data->data || data->size == 0) {
        ...
        if (format == GNUTLS_X509_FMT_PEM) {
            ...
            WGW_LOG("wc_PemToder ret: %d", ret);
            WGW_LOG("derBuf length: %d", derBuf->length);
    ...
    unsigned char* publicKeyDer, word32 publicKeySize, int* key_found)
{
    ...
    *key_found = 0;
    ...
    /* TODO: key_found should only be set when not key found. */
    if (ret != 0) {
        WGW_ERROR("Key import reported success (key_found=1) but ret=%d.", ret);
        wc_FreeRng(&ctx->rng);
        gnutls_free(ctx);
        return (ret == MEMORY_E) ? GNUTLS_E_MEMORY_ERROR :
...
static int verify_rsa_pss(int hash_type, const gnutls_datum_t *msg_data,
    const gnutls_datum_t *sig, gnutls_digest_algorithm_t hash, RsaKey *rsa_key,
    int hash_flag)
{
    int ret;
    int mgf = 0;
    ...
    /* Verify using RSA-PSS */
    ret = wc_RsaPSS_Verify(sig->data, sig->size, verify_buf, RSA_PSS_SIG_SIZE,
        hash_type, mgf, rsa_key);
    if (ret <= 0) {
    ...
    /* Verify using RSA-PSS */
    ret = wc_RsaPSS_VerifyCheck(sig->data, sig->size, verify_buf,
        RSA_PSS_SIG_SIZE, digest, hash_len, hash_type, mgf, rsa_key);
    ...
    gnutls_datum_t *pub, struct wolfssl_pk_ctx *pub_ctx)
{
    ...
    if (!priv_ctx->key.ed25519.pubKeySet) {
        WGW_ERROR("pub key was not set, can't exported");
        return GNUTLS_E_INVALID_REQUEST;
    }
    ...
    gnutls_datum_t *pub, struct wolfssl_pk_ctx *pub_ctx)
{
    ...
    if (!priv_ctx->key.ed448.pubKeySet) {
        WGW_ERROR("pub key was not set, can't be exported");
        return GNUTLS_E_INVALID_REQUEST;
    }
    ...
    const gnutls_datum_t *msg_data, gnutls_datum_t *sig)
{
    ...
    ret = wc_RsaPSS_Sign(digest, hash_len, sig_buf, sig_buf_len, hash_type, mgf,
        &ctx->key.rsa, &ctx->rng);
    if (ret < 0) {
        WGW_ERROR("RSA-PSS signing failed with code %d", ret);
        gnutls_free(sig_buf);
...
static int wolfssl_pk_sign(void *_ctx, const void *privkey,
    ...
    unsigned int flags, gnutls_sign_algorithm_t algo)
{
    struct wolfssl_pk_ctx *ctx = _ctx;
    int ret;
    enum wc_HashType hash_type;
    ...
    if (algo == GNUTLS_SIGN_ECDSA_SHA256 ||
        ...
        if (!(ctx->key.ecc.type == ECC_PUBLICKEY)) {
            ...
            ret = wc_EccPublicKeyDecode(ctx->pub_data, &idx, &ctx->key.ecc,
                ctx->pub_data_len);
            if (ret != 0) {
                WGW_ERROR("ECDSA public key import failed with code %d", ret);
                wc_ecc_free(&ctx->key.ecc);
                return GNUTLS_E_INVALID_REQUEST;
            }
        }
        ...
        if (mp_iszero(&ctx->key.rsa.n) || mp_iszero(&ctx->key.rsa.e)) {
            ...
            /* Import the public key from DER */
            ret = wc_RsaPublicKeyDecode(ctx->pub_data, &(word32){0},
        ...
        if (!pss || ret < 0) {
            ...
            ret = verify_rsa_pkcs1(hash_type, msg_data, sig, &ctx->key.rsa, 0, 0);
        }
    ...
    const void *pubkey, const gnutls_datum_t *nonce, gnutls_datum_t *secret)
{
    ...
    switch (priv_ctx->algo) {
        case GNUTLS_PK_EC:
            {
                ...
                if (!(priv_mp != NULL && !mp_iszero(priv_mp))) {
                    ...
                    if (ret != 0) {
                        WGW_ERROR("Error while importing key, failed with code %d", ret);
                        return GNUTLS_E_INVALID_REQUEST;
                    }
                }
#else
        ...
        case GNUTLS_PK_ECDH_X25519:
            {
                ...
                if (ret != 0) {
                    WGW_ERROR("X25519 public key import failed with code %d", ret);
                ...
                if (ret != 0) {
                    WGW_ERROR("X25519 shared secret generation failed with code %d", ret);
                    wc_curve25519_free(&peer_key);
                    return GNUTLS_E_INVALID_REQUEST;
                }
        ...
        case GNUTLS_PK_ECDH_X448:
            {
                ...
                if (ret != 0) {
                    WGW_ERROR("X448 public key import failed with code %d", ret);
                ...
                if (ret != 0) {
                    WGW_ERROR("X448 shared secret generation failed with code %d", ret);
                    wc_curve448_free(&peer_key);
                    return GNUTLS_E_INVALID_REQUEST;
                }
        ...
        case GNUTLS_PK_DH:
            {
                ...
                if (!(priv_ctx->priv_data_len > 0)) {
                    ...
                    ret = wc_DhExportKeyPair(&priv_ctx->key.dh, priv_ctx->priv_data,
        ...
        default:
            WGW_ERROR("PK algorithm not supported for key exchange: %d", priv_ctx->algo);
            return GNUTLS_E_INVALID_REQUEST;
    }
}
                             ...
                             gnutls_datum_t *plaintext)
{
    ...
    if (ctx->algo != GNUTLS_PK_RSA) {
        WGW_ERROR("Only RSA is supported for decryption, algorithm is: %d", ctx->algo);
        return GNUTLS_E_ALGO_NOT_SUPPORTED;
    }
...
static int wolfssl_pk_export_privkey_dh_raw(void *ctx, const void *y, const void *x)
{
    ...
    WGW_LOG("DH private key exported successfully (privSz=%u)", priv_datum->size);
    return 0;
}
